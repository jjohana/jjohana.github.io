...
test("opens the dashboard and browses the QCM bank", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "1083 QCMs in current scope" })).toBeVisible();
...
test("starts a topic drill and shows immediate feedback", async ({ page }) => {
  ...
  await expect(page.getByLabel("Question bank priority")).toHaveValue("all");
  await expect(page.getByRole("heading", { name: "974 matching QCMs" })).toBeVisible();
  await page.getByLabel("Question bank priority").selectOption("s3-market-docx");
  await expect(page.getByRole("heading", { name: "366 matching QCMs" })).toBeVisible();
...
test("starts a mock exam and opens results", async ({ page }) => {
  ...
  await expect(page.getByLabel("Question bank priority")).toHaveValue("all");
  await expect(page.getByLabel("Quality status")).toHaveValue("verified");
  await expect(page.getByText("974 matching active QCMs")).toBeVisible();
  await page.getByLabel("Question bank priority").selectOption("s3-market-docx");
  await expect(page.getByText("370 matching active QCMs")).toBeVisible();
  await expect(page.locator(".distribution-summary .metric").filter({ hasText: "Current bank" })).toContainText("370");
  await page.getByLabel("Question bank priority").selectOption("s3-regulatory-pdf");
  await expect(page.getByText("210 matching active QCMs")).toBeVisible();
  await expect(page.locator(".distribution-summary .metric").filter({ hasText: "Current bank" })).toContainText("210");
