...
test("opens the dashboard and browses the QCM bank", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "1016 QCMs in current scope" })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "220 QCMs in current scope" })).toBeVisible();
  await expect(page.locator(".regulatory-stats .metric").filter({ hasText: "Regulatory QCMs" })).toContainText("220");
...
test("starts a topic drill and shows immediate feedback", async ({ page }) => {
  ...
  await expect(page.getByLabel("Question bank priority")).toHaveValue("all");
  await expect(page.getByRole("heading", { name: "883 matching QCMs" })).toBeVisible();
  await page.getByLabel("Question bank priority").selectOption("s3-market-docx");
  await expect(page.getByRole("heading", { name: "306 matching QCMs" })).toBeVisible();
...
test("starts a mock exam and opens results", async ({ page }) => {
  ...
  await expect(page.getByLabel("Question bank priority")).toHaveValue("all");
  await expect(page.getByLabel("Quality status")).toHaveValue("verified");
  await expect(page.getByText("883 matching active QCMs")).toBeVisible();
  await page.getByLabel("Question bank priority").selectOption("s3-market-docx");
  await expect(page.getByText("306 matching active QCMs")).toBeVisible();
  await expect(page.locator(".distribution-summary .metric").filter({ hasText: "Current bank" })).toContainText("306");
  await page.getByLabel("Question bank priority").selectOption("s3-regulatory-pdf");
  await expect(page.getByText("183 matching active QCMs")).toBeVisible();
  await expect(page.locator(".distribution-summary .metric").filter({ hasText: "Current bank" })).toContainText("183");
