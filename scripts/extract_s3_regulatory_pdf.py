...
def parse_record(record: dict[str, Any]) -> tuple[dict[str, Any], list[str]]:
    ...
    question = {
        ...
        "qualityNotes": "Fresh OCR/PDF import only. This QCM is not certified until LLM content review verifies the stem, choices, correct answer, explanation, taxonomy, and current regulatory meaning.",
        "issueTypes": sorted({"OCR/transcription", *issues, *(["bad_distractors"] if letter_dependent else [])}),
